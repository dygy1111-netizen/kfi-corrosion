...
# =============================
# ③ 향후 부식 예측 및 기대수명 (모바일 대응)
# =============================
st.subheader("③ 향후 부식 예측 및 기대수명")

# ✅ 기존 col_mid_left 안에 있던 코드를 독립시킴
if 설계두께 > 0 and 측정두께 > 0 and 사용연수_내탱크 > 0:
    bins = [0, 10, 20, 30, 200]
    ...
    # ✅ 입력 부분 - Streamlit columns 대신 HTML flex로 정렬 (모바일 100% 호환)
    st.markdown("""
    <style>
    .input-flex { display: flex; flex-wrap: wrap; justify-content: space-between; gap: 8px; }
    .input-box { flex: 1 1 48%; min-width: 280px; }
    ...
    st.markdown('<div class="input-flex">', unsafe_allow_html=True)
    st.markdown('<div class="input-box">', unsafe_allow_html=True)
    산정방식 = st.selectbox("부식률 산정 방식",
                            ["평균", "중위수(P50)", "상위 75% (보수)", "상위 90% (매우 보수)"],
                            key="rate_mode_mobile")
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('<div class="input-box">', unsafe_allow_html=True)
    남은기간 = st.number_input("다음 정밀정기검사까지 남은 기간 (년)",
                            min_value=0.0, value=3.0, step=0.5,
                            key="years_left_mobile")
    st.markdown('</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
