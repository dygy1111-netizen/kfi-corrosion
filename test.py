...
if 설계두께 > 0 and 측정두께 > 0 and 사용연수_내탱크 > 0:
    ...
    # -----------------------------
    # ⑤ 부식률 산정 방식 + 남은 기간을 같은 줄에 고정 폭으로 배치
    # -----------------------------
    st.markdown(
        """
        <style>
        div[data-testid="column"] div:has(select),
    ...
    # ⑥ 한 줄 요약 표시
    st.markdown(
        f"<div style='margin-top:-5px; color:#d1d5db; font-size:0.9rem;'>"
        f"📊 <b>부식률 산정 방식:</b> {산정방식}&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;"
        f"<b>다음 정밀정기검사까지 남은 기간(년):</b> {남은기간:.1f}"
        f"</div>",
        unsafe_allow_html=True
    )
